...
void Item::display(std::ostream &
    ...
    if(full == false){
        os << std::left << std::setw(width) << getName();
        os << " [" << std::right << std::setw(6) << std::setfill('0') << m_serialNumber << "]" << std::setfill(' ') << std::endl;
    }
    
    else {
        os << std::left <<std::setw(width) << getName();
        os << " [" << std::right << std::setw(6) << std::setfill('0') << m_serialNumber << "] " << std::setfill(' ') << "Quantity: " << std::setw(width) << m_quantity << " Description: " << m_description << std::endl;
    }
}
}
